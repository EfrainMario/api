...
$app->post('/lojas/{id:[0-9]+}/promocoes/{_id:[0-9]+}/imagens', function (Request $request, Response $response, array $args)
{
    ...
    if ($imagem)
    {
        ...
        if($resultado->rowCount() > 0){
            return $response->withStatus(204)->write('Promocao criada.');
        }else{
            return $response->withStatus(400)->write('Erro ao criar a promocao.');
        }
...
$app->put('/lojas/{id:[0-9]+}/promocoes/{_id:[0-9]+}', function (Request $request, Response $response, array $args)
{
    ...
    if($jsonInArray != null)
    {
        ...
        if($resultado->rowCount() > 0){
            return $response->withStatus(201)->write('Promocao actualizada.');
        }else{
...
$app->delete('/lojas/{id:[0-9]+}/promocoes/{_id:[0-9]+}', function (Request $request, Response $response, array $args)
{
    ...
    if($id != null)
    {
        ...
        return $response->withStatus(400)->write('Um erro desconhecido ocorreu ao actualizar a promocao.');
    }
});
